...
**Diagnostic note (2026-09-07)**: User reported "Không thể trang bị vật phẩm" when equipping a newly-purchased `profile_frame` from `/profile`, even though the item was owned and `purchase_shop_item` had worked fine moments earlier. Root-cause hypothesis (could not confirm directly — this workspace's DB introspection/writes to the live project are blocked by the sandbox's auto-mode classifier, same blocker documented in "Remote Execution Status" below):
  ...
  - **Correction — actual root cause found (2026-09-07, same day)**: `level_xp` was already present; this hypothesis was wrong. User confirmed via `pg_policies` that `users_update_own`/`users_select_own`/`users_insert_own` match this migration exactly, and a manual `update public.users set equipped_profile_frame_id = ...` from the SQL Editor (superuser, bypasses RLS but still runs triggers) succeeded with no exception — so the trigger and RLS definitions are correct on remote. The Network tab showed the browser's actual PATCH request failing with **"CORS error"** (after a successful 200 preflight) only in the user's regular Chrome profile — logging out/in did not fix it, but an Incognito window (where Chrome disables extensions by default) worked immediately. Root cause: a browser extension (ad blocker/privacy tool) in the user's normal profile interferes with the cross-origin fetch to `*.supabase.co`, which the browser reports as a CORS failure even though Supabase itself is fine (verified separately: a plain anon-key PATCH from a fresh `localhost:3000` tab succeeded with no CORS issue). This is a client-side environmental issue, not a schema/RLS/trigger bug — nothing here needed a migration fix.
  - **App-side mitigation shipped**: `postgrest-js` (`PostgrestBuilder.ts`) catches any `fetch()`-level failure (extension-blocked, offline, DNS, etc.) and returns an error with an **empty `code`** and a message like `"TypeError: Failed to fetch"` — distinct from real Postgres/PostgREST errors, which always have a non-empty `code`. Added `isNetworkBlockedError(error)` in `src/lib/supabase/errors.ts` to detect this signature, and wired it into `equipShopItem`/`purchaseShopItem` (`src/lib/shop.ts`) and `updateMyProfile` (`src/lib/user-profile.ts`) so affected users see an actionable message (suggesting they disable ad-block/privacy extensions or try an Incognito window) instead of the old generic "please try again" text.
