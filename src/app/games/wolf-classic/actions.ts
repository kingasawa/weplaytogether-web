...
import { getLivePlayerProfilesByUserId, type LivePlayerProfilesLookup } from "@/lib/player-avatar-frames";
...
function mapLobbyPlayer(player: PlayerRow, liveProfiles?: LivePlayerProfilesLookup): ClassicWolfLobbyPlayer {
...
async function saveClassicGameState(
  ...
  if (Object.keys(values).length > 0) {
    await Promise.all([
      stateMutation,
      supabase.from("game_sessions").update(values).eq("id", gameId),
    ]);
    return;
  }
...
export async function createClassicWolfRoom(
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    await supabase.from("rooms").update({ host_player_id: hostPlayer.id }).eq("id", room.id);
...
export async function kickClassicWolfPlayer(
  ...
  await supabase.from("room_players").delete().eq("id", targetPlayer.id);
  await safeBroadcastWolfRoomUpdate(room.code);
...
export async function startClassicWolfGame(
  ...
  await supabase.from("rooms").update({ status: "playing", current_game_id: game.id }).eq("id", room.id);
  await safeBroadcastWolfRoomUpdate(room.code);
  await safeBroadcastWolfPlayUpdate(room.code);
